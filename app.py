...
def ui(**kwargs):
    with gr.Blocks() as interface:
        gr.Markdown(
            """
            # MusicGen
            Это ваша личная демонстрация [MusicGen](https://github.com/facebookresearch/audiocraft), простой и управляемой модели генерации музыки
            представлено на: ["Simple and Controllable Music Generation"](https://arxiv.org/abs/2306.05284)
            """
        )
        if IS_SHARED_SPACE:
            gr.Markdown("""
        ...
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    text = gr.Text(label="Ввод текста", interactive=True)
                    melody = gr.Audio(source="upload", type="numpy", label="Собственная мелодия (по желанию)", interactive=True)
                with gr.Row():
                    submit = gr.Button("Генерация", variant="primary")
                    gr.Button("Остановить").click(fn=interrupt, queue=False)
                with gr.Row():
                    background = gr.Image(source="upload", label="Изображение", type="filepath", interactive=True)
                with gr.Row():
                    model = gr.Radio(["melody", "medium", "small", "large"], label="Модель", value="melody", interactive=True)
                with gr.Row():
                    duration = gr.Slider(minimum=1, maximum=300, value=10, step=1, label="Продолжительность", interactive=True)
                with gr.Row():
                    overlap = gr.Slider(minimum=1, maximum=29, value=5, step=1, label="Перекрытие", interactive=True)
                    recondition = gr.Checkbox(False, label='Обусловить следующие куски первым куском')
                with gr.Row():
                    topk = gr.Number(label="Top-k", value=250, interactive=True)
                    topp = gr.Number(label="Top-p", value=0, interactive=True)
                    temperature = gr.Number(label="Температура", value=1.0, interactive=True)
                    cfg_coef = gr.Number(label="Классификатор бесплатных инструкций", value=3.0, interactive=True)
                with gr.Row():
            ...
            with gr.Column() as c:
                output = gr.Video(label="Сгенерированная музыка")
                seed_used = gr.Number(label='Используемый Seed', value=-1, interactive=False)
        ...
        gr.Markdown(
            ...
            Модель сгенерирует короткий музыкальный отрывок на основе предоставленного вами описания.
            Вы можете сгенерировать до 30 секунд аудио.

            Мы представляем 4 варианта модели:
            1. Melody - модель генерации музыки, способная генерировать музыкальное условие на основе текста и мелодии. **Примечание**, можно также использовать только текст.
            2. Small -- декодер-трансформер 300М, обусловленный только текстом.
            3. Medium -- трансформаторный декодер объемом 1,5 Б, работающий только с текстом.
            4. Large - 3.3B трансформаторный декодер, обусловленный только текстом (может быть OOM для самых длинных последовательностей).

            При использовании `melody` можно дополнительно предоставить эталонный звук, из которого будет извлечена широкая мелодия.
            из которого будет извлечена общая мелодия. Модель будет пытаться следовать как за описанием, так и за мелодией.

            Вы также можете использовать свой собственный GPU или Google Colab, следуя инструкциям на нашем репозитории.
            Смотрите [github.com/facebookresearch/audiocraft](https://github.com/facebookresearch/audiocraft)
